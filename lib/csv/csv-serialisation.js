...
// Space separated and zone-less: both dialects accept it, where an offset suffix would be
// rejected by MariaDB. The instant is UTC; load with the session time zone set to UTC.
const spacedTimestamp = (value) => value.toISOString().slice(0, TIMESTAMP_LENGTH).replace('T', ' ');
...
const formatters = new Map([
  ...
  [ColumnKind.Timestamp, dateOrText(spacedTimestamp)],
