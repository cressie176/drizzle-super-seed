...
const { arrayLiteral } = require('../postgres/array-literal');
...
const arrayValue = (value, column, tableKey) => {
  if (!Array.isArray(value)) return undefined;
  const formatElement = (element) =>
    formatters.get(column.element.kind)(element, column.element, tableKey) ?? unserialisable(tableKey, column, element);
  return arrayLiteral(value, tableKey, column, formatElement);
};
...
const serialiseValue = (tableKey, column, value, { nullToken }) => {
  ...
  const formatted = formatters.get(column.kind)(value, column, tableKey) ?? unserialisable(tableKey, column, value);
