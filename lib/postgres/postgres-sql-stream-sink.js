...
const createPostgresSqlStreamSink = ({
  writable,
  ...
  return {
    ...
    async prepare(schema) {
      // Once, at the head of the stream: the whole script goes to one psql, so a failed COPY
      // should stop the load rather than let the rest run and exit 0.
      await writeChunk(writable, SCRIPT_PROLOGUE);
      if (tableLogging !== TableLogging.SetUnlogged) return;
      const { tables: ordered, deferredForeignKeys } = orderTablesByDependency(schema);
      await writeChunk(writable, setUnloggedScript(ordered, deferredForeignKeys));
    },
