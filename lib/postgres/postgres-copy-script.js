...
// psql reports a failed statement and carries on, exiting 0, so a COPY that violates a constraint
// loses its whole table while the load still reports success to the shell. The orchestrator sets
// this, but a file loaded on its own, or piped straight into psql, would not be covered. These
// files already contain psql's own copy-from-stdin markers, so they were never portable to a
// driver, and a meta-command at the head costs nothing.
const SCRIPT_PROLOGUE = '\\set ON_ERROR_STOP on\n';
...
// A pulled nextval default carries its sequence name; a real serial or identity column leaves
// it to the catalogue.
const sequenceReference = (table, column) =>
  column.sequenceName === undefined
    ? `pg_get_serial_sequence('${qualified(table)}', '${column.name}')`
    : `'${column.sequenceName}'`;

const setval = (table, column) =>
  [
    `SELECT setval(${sequenceReference(table, column)},`,
...
const keptLoggedKeys = (orderedTables, deferredForeignKeys) => {
  ...
  for (const key of kept) {
    ...
    for (const foreignKey of table.foreignKeys) kept.add(foreignKey.referencedTableKey);
    for (const foreignKey of table.compositeForeignKeys) kept.add(foreignKey.referencedTableKey);
  }
  return kept;
};
