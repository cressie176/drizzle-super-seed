...
// The factory each mistake was made in is the one the message names, so the shared body takes it
// rather than reporting weightedPickFrom to someone who called weightedPick.
const pickerFrom = (factory, entries) => {
  requireEntries(factory, entries);
  const weights = entries.map(({ weight }) => weight);
  requireDrawableWeights(factory, weights);
  const values = entries.map(({ value }) => value);
  const totals = runningTotals(weights);
  return (context) => values[indexByWeight(totals, context.random)];
};

const weightedPickFrom = (entries) => pickerFrom('weightedPickFrom', entries);

const asEntries = (weights) => Object.entries(weights).map(([value, weight]) => ({ value, weight }));

const weightedPick = (weights) => pickerFrom('weightedPick', asEntries(weights));

const harmonicWeights = (ranks, skew) => Array.from({ length: ranks }, (_, index) => (index + 1) ** -skew);

const zipfInteger = (min, max, skew = DEFAULT_SKEW) => {
  requireWholeRange('zipfInteger', min, max);
  requireSkew('zipfInteger', skew);
...
const normalInteger = (mean, stdDev, clamp = NO_CLAMP) => {
  requireMean('normalInteger', mean);
  requireDeviation('normalInteger', stdDev);
  requireOrderedClamp('normalInteger', clamp);
  return (context) => clampedTo(clamp, Math.round(mean + standardNormal(context.random) * stdDev));
};
