...
// The first 32 bits of a UUID, because mulberry32 consumes exactly that (seed >>> 0): a wider
// seed would have bits the source discards, and a reported seed that did not reproduce its run.
...
// A primary key admits no NULL, so it behaves as NULLS NOT DISTINCT for tracking purposes. A
// table without one contributes no constraint: an empty column list would make a constraint whose
// tuple is the same for every row, and the second row a collision.
const constraintColumnLists = (table) => [
  ...(table.primaryKey.length > 0 ? [{ columns: table.primaryKey, nullsNotDistinct: true }] : []),
...
const plannedConstraint = (entries, { columns: columnNames, nullsNotDistinct }) => {
  const constrained = columnNames.map((columnName) => entries.find((entry) => entry.column.name === columnName));
  return {
    entries: constrained,
    columns: constrained.map((entry) => entry.column),
    nullsNotDistinct,
    emitted: new Set(),
  };
};

const distinctColumnLists = (constraints) => [
  ...new Map(constraints.map((constraint) => [constraint.columns.join(), constraint])).values(),
];
...
const plannedConstraints = (table, entries, overridden) =>
  distinctColumnLists(constraintColumnLists(table))
    .map((constraint) => plannedConstraint(entries, constraint))
...
// An undefined or null item is a row with no override, same as an empty object: the natural
// spelling for "only override the second row" should not throw from Object.keys.
...
// Every dialect this library supports defaults to NULLS DISTINCT: a unique constraint admits any
// number of rows with a NULL in it. A tuple containing one is therefore neither a collision nor
// worth recording, unless the constraint was declared NULLS NOT DISTINCT.
const isTracked = (constraint, values) => constraint.nullsNotDistinct || !values.some(isAbsent);

const clashingConstraint = (planned, row) =>
  planned.constraints.find((constraint) => {
    const values = constraintTuple(constraint, row);
    return isTracked(constraint, values) && constraint.emitted.has(tupleKey(values));
  });

const recordConstraints = (planned, row) => {
  for (const constraint of planned.constraints) {
    const values = constraintTuple(constraint, row);
    if (isTracked(constraint, values)) constraint.emitted.add(tupleKey(values));
  }
};
...
const requirePrimaryKeyForPerParent = (schema, plannedTables) => {
  for (const planned of plannedTables.filter((each) => each.perParent)) {
    const parent = schema.tables.get(planned.perParent.parentKey);
    if (parent.primaryKey.length > 0) continue;
    throw new MissingPrimaryKeyError({ reason: 'perParent', table: parent.key, childTable: planned.table.key });
  }
};

const requirePrimaryKeyForDeferredPasses = (passes) => {
  for (const pass of passes) {
    const { table } = pass.planned;
    if (table.primaryKey.length > 0) continue;
    throw new MissingPrimaryKeyError({
      reason: 'deferredColumn',
      table: table.key,
      column: pass.entry.column.propertyName,
    });
  }
};
...
const clashesDeferred = (constraint, value) =>
  isTracked(constraint, [value]) && constraint.emitted.has(tupleKey([value]));
...
const settledDeferredValue = (planned, entry, rowIndex, pool, run) => {
  ...
  // A NULL is never a collision under NULLS DISTINCT, so it returns immediately rather than
  // burning a retry and suppressing the null rate the deferred pass is supposed to produce.
  for (let attempt = 0; clashesDeferred(constraint, value) && attempt < MAX_UNIQUE_ATTEMPTS; attempt++) {
    value = chosenDeferredValue(pool, run);
  }
  if (clashesDeferred(constraint, value)) {
    throw new UniqueConstraintExhaustedError({
      table: planned.table.key,
  ...
  if (isTracked(constraint, [value])) constraint.emitted.add(tupleKey([value]));
  return value;
};
...
const generate = async (config, sink) => {
  ...
  requirePrimaryKeyForPerParent(schema, plannedTables);
  requirePrimaryKeyForDeferredPasses(passes);
  requireDeferredSupport(passes, sink);
  await sink.prepare?.(schema);
