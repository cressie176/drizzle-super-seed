...
const {
  GenerationError,
  GenerationPlanRefusalsError,
  MissingColumnRuleError,
  MissingParentCountError,
  MissingTableRulesError,
  UnseededTableCountedError,
  UnseededTableRuledError,
  NotNullSelfReferenceError,
  CheckConstrainedColumnRuleRequiredError,
  CheckConstrainedForeignKeyRuleRequiredError,
...
// Table-level annotations are symbol keys on the rules object, never strings, so they can never
// collide with a column: a schema is free to declare a column called `unseeded`, and the two keys
// sit side by side untouched. Symbol keys are also skipped by Object.keys, so every place that
// walks a rules object looking for columns keeps seeing only columns.
const unseeded = Symbol('unseeded');

const structuralDefaults = Symbol('structuralDefaults');

const annotated = (tableRules, annotation) => tableRules[annotation] === true;

// `unseeded` was once the whole rules value for a table rather than an annotation on it. That form
// still works and still means the same thing, but it is no longer documented: table-level
// annotations are keys now, so that a table can carry more than one and so that new annotations
// have somewhere to go. Warned once per table, through Node's own deprecation channel, so a run
// that has not been migrated says so without failing.
const LEGACY_UNSEEDED = [
  "Passing `unseeded` as a table's rules is deprecated.",
  'Write `{ [unseeded]: true }` instead, which leaves room for other table annotations.',
].join(' ');

const isLegacyUnseeded = (tableRules, table) => {
  if (tableRules !== unseeded) return false;
  process.emitWarning(`${LEGACY_UNSEEDED} Table: ${table.key}.`, 'DeprecationWarning');
  return true;
};
...
const DEFAULT_ARRAY_MAX_LENGTH = 3;

const arrayDepth = (column) => {
  let depth = 0;
  let current = column;
  while (current.kind === ColumnKind.Array) {
    depth += 1;
...
// A short array of whatever the element kind generates. Each level takes one length for the
// whole value, not one per sub-array, because PostgreSQL rejects a ragged literal: sub-arrays
// must have matching dimensions.
const shortArray = (column) => {
  const { depth, leaf } = arrayDepth(column);
  const element = kindDefault(leaf);
  return (context) => {
    const lengths = Array.from({ length: depth }, () =>
      context.random.intBetween(DEFAULT_ARRAY_MIN_LENGTH, DEFAULT_ARRAY_MAX_LENGTH),
    );
    const build = (level) =>
      level === depth ? element(context) : Array.from({ length: lengths[level] }, () => build(level + 1));
    return build(0);
  };
};
...
const defaultsByKind = new Map([
  ...
  [ColumnKind.Array, shortArray],
]);

const kindDefault = (column) => defaultsByKind.get(column.kind)(column);
...
// A check constraint's predicate is opaque SQL, so nothing here can tell whether generated data
// would satisfy it, and the empirical answer on a real schema is that it usually does not: the
// violation aborts the whole COPY, so the table loads nothing at all. Refusing up front names the
// constraint and quotes the predicate, which is the difference between a rule to write and a
// database error to decipher. An explicit rule of any kind satisfies this, since only invented
// values are refused.
const checkConstraining = (table, column) =>
  table.checkConstraints.find((constraint) => constraint.columns.includes(column.name));

const requireUncheckedColumn = (table, column) => {
  const constraint = checkConstraining(table, column);
  if (!constraint) return;
  throw new CheckConstrainedColumnRuleRequiredError({
    table: table.key,
    column: column.propertyName,
...
// The exemption below assumes at least one column the check names will demand a rule, so the
// user meets the constraint before the load does. A check naming only foreign keys and
// engine-numbered columns would demand nothing from anyone, and the MusicBrainz validation
// (#41) showed how that ends: entity0 <> entity1 over two random picks from one pool, refused
// by the database mid-COPY. For such a check the foreign keys it names are refused instead,
// since their rules are the only ones that could carry it.
const exemptColumnNames = (table) => {
  const names = new Set(table.foreignKeys.map((foreignKey) => foreignKey.columnName));
  for (const composite of table.compositeForeignKeys) for (const name of composite.columns) names.add(name);
  for (const column of table.columns) if (column.sequenceOwned) names.add(column.name);
  return names;
};

const requireUncheckedReference = (table, column) => {
  const exempt = exemptColumnNames(table);
  const constraint = table.checkConstraints.find(
    (each) => each.columns.includes(column.name) && each.columns.every((name) => exempt.has(name)),
  );
  if (!constraint) return;
  const propertyNames = new Map(table.columns.map((each) => [each.name, each.propertyName]));
  throw new CheckConstrainedForeignKeyRuleRequiredError({
    table: table.key,
    column: column.propertyName,
    constraint: constraint.name,
    predicate: constraint.predicate,
    columns: constraint.columns.map((name) => propertyNames.get(name)),
  });
};
...
const resolveStructurally = (table, column) => {
  ...
  const foreignKey = foreignKeyOf(table, column);
  if (foreignKey) requireUncheckedReference(table, column);
  ...
  // Only invented values are refused. A foreign key takes its value from a real parent row and a
  // sequence from the engine, neither of which is a guess, and neither of which the user could
  // express by hand without giving up the reference. A check over those columns is left alone,
  // unless it names no column that could ever be refused, which the reference refusal above owns.
  requireUncheckedColumn(table, column);
...
const requireRule = (table, column, tableRules) => {
  ...
  // The table asked for structural defaults on everything it did not name, so an unnamed column
  // is the request being honoured rather than an omission.
  if (annotated(tableRules, structuralDefaults)) return;
...
// An unseeded table generates nothing, so anything else in its rules object contradicts it:
// column rules that can never run, or a second annotation that disagrees with the first.
const requireNothingBesidesUnseeded = (table, tableRules) => {
  const declared = [
    ...Object.keys(tableRules),
    ...(annotated(tableRules, structuralDefaults) ? ['structuralDefaults'] : []),
  ];
  if (declared.length === 0) return;
  throw new UnseededTableRuledError({ table: table.key, declared });
};

const rulesFor = (rules, table) => {
  const tableRules = rules[table.key];
  if (!tableRules) throw new MissingTableRulesError({ table: table.key });
  if (isLegacyUnseeded(tableRules, table)) throw new UnseededTableCountedError({ table: table.key });
  if (annotated(tableRules, unseeded)) throw new UnseededTableCountedError({ table: table.key });
  return tableRules;
};
...
// A refusal is a rule the user still has to write, and one per run at MusicBrainz scale is a
// wall (#63): the walk visits every table and column regardless, so the refusals it raises are
// collected and thrown together. Anything that is not a refusal is a bug and propagates as
// itself, immediately.
const collecting = (refusals, resolve) => {
  try {
    return resolve();
  } catch (error) {
...
// A single decision can refuse several columns: each member of a composite foreign key, or each
// foreign key an all-exempt check names. Those entries name the same tuple and the same remedy,
// so the report keeps one of them; genuinely distinct refusals always differ by column.
const refusalKey = (refusal) =>
  [refusal.name, refusal.table, refusal.constraint ?? '', (refusal.columns ?? [refusal.column]).join(',')].join('|');

const distinctRefusals = (refusals) => {
  const seen = new Map();
  for (const refusal of refusals) {
    const key = refusalKey(refusal);
    if (!seen.has(key)) seen.set(key, refusal);
  }
  return [...seen.values()];
};

const throwRefusals = (refusals) => {
  const distinct = distinctRefusals(refusals);
  if (distinct.length === 1) throw distinct[0];
  if (distinct.length > 1) throw new GenerationPlanRefusalsError({ refusals: distinct });
};

const planTable = (table, rules, counts, deferred, refusals) => {
  const tableRules = collecting(refusals, () => {
    const found = rulesFor(rules, table);
    requireKnownColumns(table, found);
    return found;
  });
  if (tableRules === null) return null;
  return table.columns.map((column) =>
    collecting(refusals, () => planColumn(table, column, tableRules, counts, deferred)),
  );
};

// Coherence is checked for every table that declares rules, not only the ones being generated: a
// contradiction is a mistake in the rules whether or not this run happens to reach that table.
const requireCoherentRules = (schema, rules) => {
  for (const table of schema.tables.values()) {
    const tableRules = rules[table.key];
    if (!tableRules || tableRules === unseeded) continue;
    if (!annotated(tableRules, unseeded)) continue;
    requireNothingBesidesUnseeded(table, tableRules);
  }
};
...
const resolveGenerationPlan = (schema, rules, counts, deferredForeignKeys = []) => {
  requireCoherentRules(schema, rules);
  requireCountedTablesDeclared(schema, counts);
  const refusals = [];
  const plan = new Map(
    countedTables(schema, counts).map((table) => [
      table.key,
      planTable(table, rules, counts, deferredForeignKeys, refusals),
    ]),
  );
  throwRefusals(refusals);
  return plan;
};

module.exports = { PlanSource, resolveGenerationPlan, structuralDefault, structuralDefaults, unseeded };
