...
const wholeNumber = () => randomInteger(0, DEFAULT_INTEGER_MAX);
...
const structuralDefaults = new Map([
  ...
  [ColumnKind.BigInt, wholeNumber],
  [ColumnKind.Decimal, decimalString],
  [ColumnKind.Real, realNumber],
  [ColumnKind.Text, boundedWords],
  [ColumnKind.Boolean, eitherBoolean],
  [ColumnKind.Uuid, anyUuid],
  [ColumnKind.Date, recentDateString],
  [ColumnKind.Timestamp, recentTimestamp],
...
const resolveStructurally = (table, column) => {
  ...
  if (column.sequenceOwned) return engineManaged(rowNumber);
...
const requireParentCount = (table, column, source, counts) => {
  if (source !== PlanSource.EngineManaged) return;
...
const planTable = (table, rules, counts) => {
  const tableRules = rulesFor(rules, table);
  requireKnownColumns(table, tableRules);
  return table.columns.map((column) => planColumn(table, column, tableRules, counts));
};
...
const resolveGenerationPlan = (schema, rules, counts) => {
  requireCountedTablesDeclared(schema, counts);
  return new Map(countedTables(schema, counts).map((table) => [table.key, planTable(table, rules, counts)]));
};
