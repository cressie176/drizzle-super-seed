...
// drizzle-orm 1.0.0 dissolved the wrapper array column: the element column itself carries a
// dimensions count and stays otherwise scalar, so the array and its element are described from
// the same object, one dimension at a time. Vectors declare dimensions of their own on 0.x, and
// 0.x wrapper arrays never do, so the declared kind decides before the count is trusted.
const flatDimensions = (kind, column) =>
  kind === ColumnKind.Vector || kind === ColumnKind.Array ? 0 : (column.dimensions ?? 0);

const flatLeaf = (dialect, tableKey, propertyName, column) => {
  const kind = kindOf(dialect, tableKey, propertyName, column);
  return { kind, jsType: jsTypeOf(column.dataType), ...dialect.valueRange(column), ...dialect.detail(kind, column) };
};

const flatElement = (dialect, tableKey, propertyName, column, remaining) => ({
  element:
    remaining > 1
      ? {
          kind: ColumnKind.Array,
          jsType: 'array',
          ...flatElement(dialect, tableKey, propertyName, column, remaining - 1),
        }
      : flatLeaf(dialect, tableKey, propertyName, column),
});

const scalarDescription = (dialect, tableKey, propertyName, column, kind) => ({
...
const canonicalColumn = (dialect, tableKey, propertyName, column, casing) => {
  const declaredKind = kindOf(dialect, tableKey, propertyName, column);
  const dimensions = flatDimensions(declaredKind, column);
  const kind = dimensions > 0 ? ColumnKind.Array : declaredKind;
  return {
    name: databaseName(column, casing),
    propertyName,
    kind,
    jsType: dimensions > 0 ? 'array' : jsTypeOf(column.dataType),
    ...
    ...(dimensions > 0
      ? flatElement(dialect, tableKey, propertyName, column, dimensions)
      : scalarDescription(dialect, tableKey, propertyName, column, kind)),
  };
};
