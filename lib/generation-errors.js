...
const unresolvableRelationMessages = {
  ...
  through: ({ table, relation, referencedTable }) =>
    [
      `Relation ${table}.${relation} cannot be resolved: it joins ${referencedTable} through another`,
      "table, and in-memory navigation does not run joins. Navigate the junction table's own relations instead.",
    ].join(' '),
  filtered: ({ table, relation, referencedTable }) =>
    [
      `Relation ${table}.${relation} cannot be resolved: it declares a where filter, and in-memory`,
      `navigation does not apply filters. Declare an unfiltered relation to ${referencedTable}, or filter the rows yourself.`,
    ].join(' '),
};
...
const raggedArrayMessage = ({ table, column, lengths }) =>
  [
    `Column ${table}.${column} produced a nested array whose sub-arrays have lengths`,
    `${lengths.join(' and ')}. PostgreSQL requires them to match.`,
    'Generate sub-arrays of one length.',
  ].join(' ');

class RaggedArrayError extends GenerationError {
  constructor({ table, column, lengths }) {
    super(raggedArrayMessage({ table, column, lengths }));
    this.table = table;
    this.column = column;
...
const unseededTableRuledMessage = ({ table, declared }) =>
  [
    `Table ${table} is annotated unseeded, so it generates nothing and must declare nothing else,`,
    `but it also declares ${declared.join(', ')}.`,
    'Remove the annotation to seed the table, or remove everything beside it.',
  ].join(' ');

class UnseededTableRuledError extends GenerationError {
  constructor({ table, declared }) {
    super(unseededTableRuledMessage({ table, declared }));
    this.table = table;
    this.declared = declared;
  }
}

const checkConstrainedColumnRuleRequiredMessage = ({ table, column, constraint, predicate }) =>
  [
    `Column ${table}.${column} is constrained by the check ${constraint} (${predicate}),`,
    'whose predicate is opaque SQL that a structural default cannot be expected to satisfy.',
    'Write a rule for it, or for the columns it relates, which the check accepts.',
  ].join(' ');

class CheckConstrainedColumnRuleRequiredError extends GenerationError {
  constructor({ table, column, constraint, predicate }) {
    super(checkConstrainedColumnRuleRequiredMessage({ table, column, constraint, predicate }));
    this.table = table;
    this.column = column;
...
const checkConstrainedForeignKeyRuleRequiredMessage = ({ table, column, constraint, predicate, columns }) =>
  [
    `Column ${table}.${column} is a foreign key constrained by the check ${constraint} (${predicate}),`,
    'and every column the check names is a foreign key or engine-numbered, so no rule would otherwise',
    'be demanded and a random parent reference cannot be expected to satisfy the predicate.',
    `Write rules for ${columns.join(', ')} which keep the references valid together, such as row`,
    'numbers into their parent tables.',
  ].join(' ');

class CheckConstrainedForeignKeyRuleRequiredError extends GenerationError {
  constructor({ table, column, constraint, predicate, columns }) {
    super(checkConstrainedForeignKeyRuleRequiredMessage({ table, column, constraint, predicate, columns }));
    this.table = table;
    this.column = column;
    this.constraint = constraint;
    this.predicate = predicate;
...
// The one deliberately multi-line message: it is a report rather than a sentence, and the value
// of collecting refusals is seeing them together, grouped the way the rules file is organised.
// Each entry keeps the message it would have carried alone, so nothing is lost to the summary.
const generationPlanRefusalsMessage = ({ refusals }) => {
  const byTable = new Map();
  for (const refusal of refusals) {
    const entries = byTable.get(refusal.table) ?? [];
    byTable.set(refusal.table, entries);
    entries.push(refusal);
  }
  const lines = [
    `Resolving the generation plan raised ${refusals.length} refusals across ${byTable.size} tables,`,
    'each one a rule to write:',
  ];
  const report = [lines.join(' ')];
  for (const [table, entries] of byTable) {
    report.push('', `${table}:`, ...entries.map((entry) => `- ${entry.message}`));
  }
  return report.join('\n');
};

class GenerationPlanRefusalsError extends GenerationError {
  constructor({ refusals }) {
    super(generationPlanRefusalsMessage({ refusals }));
