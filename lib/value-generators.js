...
// Embedding models emit unit vectors, and cosine distance assumes them, so components are drawn
// over [-1, 1) and scaled to unit length. A zero draw would divide by zero, so it retries.
const unitVector = (random, dimensions) => {
  const components = Array.from({ length: dimensions }, () => random.next() * 2 - 1);
  const magnitude = Math.sqrt(components.reduce((total, part) => total + part * part, 0));
  if (magnitude === 0) return unitVector(random, dimensions);
  return components.map((part) => part / magnitude);
};

const randomVector = (dimensions) => {
  requireDimensions('randomVector', dimensions);
  return (context) => unitVector(context.random, dimensions);
};
