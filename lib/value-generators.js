...
const randomInteger = (min, max) => (context) => context.random.intBetween(min, max);

const randomDecimalString = (min, max, scale) => (context) =>
  (min + context.random.next() * (max - min)).toFixed(scale);

const randomBoolean =
  (probabilityTrue = DEFAULT_PROBABILITY_TRUE) =>
  (context) =>
    context.random.chance(probabilityTrue);
...
const randomWords =
  ({ minLength = DEFAULT_MIN_WORDS_LENGTH, maxLength = DEFAULT_MAX_WORDS_LENGTH } = {}) =>
  (context) =>
    wordsOfLength(context.random, context.random.intBetween(minLength, maxLength));
...
const randomDateBetween = (from, to) => (context) => dateBetween(context.random, from, to);
...
const optional =
  (generator, nullProbability = DEFAULT_NULL_PROBABILITY) =>
  (context) =>
    context.random.chance(nullProbability) ? null : generator(context);
