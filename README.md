...
- [Rules](#rules)
  ...
  - [Third-party generators: faker and zocker](#third-party-generators-faker-and-zocker)
- [Overrides](#overrides)
...
Every example in this document uses one domain: holiday parks. Parks contain pitches, holiday homes sit on pitches and belong to owners, and lettings book homes for date ranges. The same schema runs through the test suite and the [examples](#examples) directory, so any snippet here can be found working there.
...
Relative date generators use a reference date, which is recorded alongside the seed. The same seed, configuration and reference date produce byte-identical output. The file sinks record this information in `manifest.json` as well as returning it in the report.

The manifest is itself a pure function of the inputs: nothing volatile is written into it, so an unchanged run rewrites the whole output directory byte for byte. That is what makes the directory safe to content-address, to cache on an input hash, and to diff against a previous run to decide whether to regenerate. Wall clock facts about the run, its duration included, are on the returned report only. Each file sink's manifest also carries a `contentHash` over the files it wrote, ready to use as a cache key for anything built from them.
...
### Third-party generators: faker and zocker

drizzle-super-seed deliberately depends on no third-party data generator, but works alongside any of them: [@faker-js/faker](https://fakerjs.dev/) for realistic names and addresses, [zocker](https://zocker.sigrist.dev/) for JSON matching a zod schema, or anything else you can call from a rule. Choose one seed, give it to every library involved, and call the generator inside `derive` rules:
...
The same pattern fills `jsonb` columns with structured values. A JSON column declares nothing about its shape, so the structural default is deliberately trivial (an empty object) and the shape belongs in a rule. Describe it once with [zod](https://zod.dev/) and let [zocker](https://zocker.sigrist.dev/) generate matching values:
...
const amenities = z.object({
  ...
  hotTub: z.boolean(),
  sleeps: z.number().int().min(2).max(8),
});
...
const parkRules = {
  ...
  amenities: derive((park, context) => zocker(amenities).setSeed(seed + context.rowIndex).generate()),
} satisfies TableRules<typeof schema.parks>;
```

One difference from faker: a zocker seed pins the whole generated value rather than advancing a stream, so seed it per row (`seed + context.rowIndex`) or every row receives the same object.
...
Overrides are positional, so to touch only a later row, pass an empty object (or `undefined`) for each row before it: `overrides: { owners: [{}, { fullName: 'Ben Patel' }] }` pins the second owner's name and generates everything else, first owner included.
...
**PostgreSQL only:** for a table containing a `GENERATED ALWAYS AS IDENTITY` column, `overrideSystemValue` is `true`, because PostgreSQL rejects explicit IDs in an ordinary insert and the handler must use Drizzle's `.overridingSystemValue()` modifier. MariaDB and SQLite have no equivalent restriction, so for their tables the flag is always `false` and the branch never runs.
...
Batches hold up to 1,000 rows by default. Set `batchSize` on the generate config to change that; it is a memory and back-pressure granularity shared by every sink, so a bigger batch means fewer handler calls at the cost of more rows held at once, and it never changes what is generated.
...
docker/initdb/
  ...
  seed-0000_set_unlogged.sql  # makes every table UNLOGGED before its data arrives
  seed-0010_parks.sql         # one file per table, in foreign-key-safe order
  seed-0020_pitches.sql
  ...
  seed-9990_finalise.sql      # updates sequences and runs ANALYZE
  manifest.json          # records how the dataset was generated
```

Every generated name carries a prefix, `seed-` by default, so the whole set sorts **after** any digit-prefixed migration sharing the directory. That matters because a number alone cannot guarantee it: comparing a five-digit name with a four-digit one reaches the separator against a digit and loses, so `10010_parks.sql` sorts before `2000_migration.sql` in both C and en_US collations. A leading letter beats every digit in both. Pass `filePrefix` to change it, including to an empty string when the directory holds nothing else; a prefix containing a path separator is refused. Numbers run from `0010` in steps of ten, giving 998 files per run: unlike migrations, which accumulate for the life of a project, these are replaced wholesale each time, so the sequence only has to hold one schema's tables and its deferred files.

`manifest.json` records the seed, reference date, row counts, trigger handling, table logging, generated files and a content hash. Use it to reproduce the files or verify the row counts after loading.

`contentHash` is a `sha256:` digest folded over the files the manifest lists, in that order, and it is what `sha256sum` produces over the same list:

```sh
cd docker/initdb
sha256sum $(jq -r '.files[]' manifest.json) | sha256sum
```

Recomputing it needs no help from this library, which is the point: it is a cache key you can check. It covers the listed payload files only. The manifest carries the answer so it cannot hash itself, and the orchestrator is derivable from the file list. Use it to key whatever is built from the directory, a seeded image for instance, without reading the files back yourself.
...
Also by default, `seed-0000_set_unlogged.sql` makes every table of the schema `UNLOGGED` before any data arrives: no write-ahead logging, the fastest possible load, and the right trade for a disposable database. Three things to know:
...
`seed-9990_finalise.sql` moves every sequence beyond its highest generated ID and runs `ANALYZE`. Subsequent inserts therefore receive unused IDs, and the query planner receives statistics for the loaded data.
...
of extended `INSERT` statements, a `seed-9990_finalise.sql` running `ANALYZE TABLE`, a `load.mysql`
...
- Rows are written as extended `INSERT` statements, 1,000 per statement by default. Each table's
  rows always land in one file; `rowsPerStatement` only sets how they are chunked into statements
  within it. Raising it repeats the `INSERT INTO ... VALUES` header less often, so the same data
  makes a slightly smaller file which loads slightly faster; the ceiling is the server's
  `max_allowed_packet` (64 MB by default), and the manifest records the size used.
...
- **A trimmed module meets the UNLOGGED default.** Tables you removed from the module still exist in the database, and one of them referencing a generated table fails `seed-0000_set_unlogged.sql` with the documented 42P16 error. Trimmed-module loads generally want `tableLogging: TableLogging.LeaveLogged`.
...
- Array columns are supported on PostgreSQL, at any depth. `structuralDefault` generates a short array of whatever the element type produces, so a `varchar(64)[]` respects its length and a `pgEnum(...).array()` picks from its values. Rules must produce a JavaScript array or null; a pre-formatted string is refused, since it could only smuggle a malformed literal past the checks. Note that PostgreSQL does not enforce dimensionality (`text[][]` is the same type as `text[]`), but it does require nested arrays to be rectangular: a rule producing ragged sub-arrays raises `RaggedArrayError` naming the column, rather than reaching the database. MariaDB and SQLite have no array type, so their array columns are still refused.
- Exotic types not listed here are rejected with a clear error naming the column.
...
The [examples](examples) directory holds twelve self-contained packages, each one you can read, or copy, on its own: seven demonstrating a sink apiece on one holiday-park domain (parks, pitches, holiday homes, owners, lettings), and five seeding real schemas - three introspected from a live database, two drizzle-native.
...
| [examples/real-world/musicbrainz](examples/real-world/musicbrainz) | MusicBrainz at 372 tables: 770 foreign keys, 663 checks, partitions, rules generated by classifier |
