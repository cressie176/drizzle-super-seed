...
- `examples/real-world/musicbrainz` seeds the MusicBrainz schema at its full 372-table scale:
  770 foreign keys, 663 CHECK constraints, 226 unique indexes, LIST partitions seeded directly
  through their pull-invisible parents, an ICU collation and a cube column, baked into a
  Postgres image with a catalogue-driven audit of every foreign key. The rules are generated by
  a predicate classifier rather than written by hand - 161 of the 372 tables need nothing
  beyond the structuralDefaults skeleton - which is the pattern to crib when a schema's checks
  number in the hundreds. Grown from the validation exercise in #41, reversing its
  not-an-example-candidate call at the maintainer's request; the DDL is fetched at generate
  time pinned to an upstream commit, because musicbrainz-server is GPL-2.0-or-later and this
  repository is MIT.
