...
describe('the csv file sink', () => {
  ...
  it('writes one numbered file per table and a manifest, in dependency order', async () => {
    ...
    deq((await readdir(directory)).sort(), ['manifest.json', 'seed-00010_parks.csv', 'seed-00020_pitches.csv']);
  });

  it('writes a header of database column names, then the rows', async () => {
    await generateCsv();
    const lines = (await readFile(join(directory, 'seed-00010_parks.csv'), 'utf8')).split('\n');
  ...
  it('omits the header when asked', async () => {
    ...
    const [first] = (await readFile(join(directory, 'seed-00010_parks.csv'), 'utf8')).split('\n');
  ...
  it('records the run in the manifest', async () => {
    ...
    deq(recorded.files, ['seed-00010_parks.csv', 'seed-00020_pitches.csv']);
  ...
  it('accepts any dialect, because csv has none', async () => {
    ...
    ok((await readdir(directory)).includes('seed-00010_readings.csv'));
  });
  ...
  it('resolves a cyclic schema, writing final values with no patch step visible', async () => {
    ...
    const lines = (await readFile(join(directory, 'seed-00010_halls.csv'), 'utf8')).trim().split('\n').slice(1);
  ...
  it('still writes every table of a cyclic run, streamed or buffered', async () => {
    ...
    deq(names, ['manifest.json', 'seed-00010_parks.csv', 'seed-00020_halls.csv', 'seed-00030_wardens.csv']);
  });
});
