...
export const generateMigrations = async (): Promise<string> => {
  const out = join(packageRoot, 'drizzle');
  await rm(out, { recursive: true, force: true });
  await run('npx', ['drizzle-kit', 'generate'], { cwd: packageRoot });
  return out;
};

// drizzle-kit separates statements with a `--> statement-breakpoint` line, which is a SQL comment
// to PostgreSQL and SQLite but not to MySQL, so files are split into statements rather than
// executed whole.
